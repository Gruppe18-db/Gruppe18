...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['speichern'])) {
    ...
    if ($VornameF && $NachnameF) {
        ...
        $success = "Fahrer wurde erfolgreich angelegt.";
    } else {
...
<form method="post">
    <input type="hidden" name="MitarbeiterID" value="<?= $editCyclist['MitarbeiterID'] ?? '' ?>">

    Vorname: 
    <input name="VornameF" value="<?= htmlspecialchars($editCyclist['VornameF'] ?? '') ?>" 
    required><br><br>

    Nachname: 
    <input name="NachnameF" value="<?= htmlspecialchars($editCyclist['NachnameF'] ?? '') ?>" 
    required><br><br>

    Straße: 
    <input name="Strasse" value="<?= htmlspecialchars($editCyclist['Strasse'] ?? '') ?>"
    ><br><br>

    Hausnummer: 
    <input name="Hausnummer" value="<?= htmlspecialchars($editCyclist['Hausnummer'] ?? '') ?>"
    ><br><br>

    PLZ: 
    <input name="PLZ" value="<?= htmlspecialchars($editCyclist['PLZ'] ?? '') ?>"
    ><br><br>

    Ort: 
    <input name="Ort" value="<?= htmlspecialchars($editCyclist['Ort'] ?? '') ?>"
    ><br><br>

    Telefonnummer: 
    <input name="Telefonnummer" value="<?= htmlspecialchars($editCyclist['Telefonnummer'] ?? '') ?>"
    ><br><br>
...
<table border="1" cellpadding="5">
    ...
    <tr>
        ...
        <td>
            <a 
            href="?edit=<?= $cyclist['MitarbeiterID'] ?>">Bearbeiten
            </a> |
            <a 
            href="?delete=<?= $cyclist['MitarbeiterID'] ?>" 
            onclick="return confirm('Sind Sie sicher?')">Löschen
            </a>
        </td>
    </tr>
    <?php 
